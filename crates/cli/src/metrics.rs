//! `xiaomimimo metrics` — reads the audit log and session/task stores and prints
//! a human-readable usage rollup.
//!
//! Data sources:
//! - `~/.xiaomimimo/audit.log`   — one JSON line per event (approvals, credentials)
//! - `~/.xiaomimimo/sessions/`   — saved session JSON files (tool call history)
//! - `~/.xiaomimimo/tasks/runtime/events/` — runtime thread JSONL event streams
...
/// Arguments accepted by `xiaomimimo metrics`.
...
fn xiaomimimo_home() -> PathBuf {
    // Respect XIAOMIMIMO_HOME env override; fall back to ~/.xiaomimimo.
    if let Ok(v) = std::env::var("XIAOMIMIMO_HOME")
        && !v.is_empty()
    {
