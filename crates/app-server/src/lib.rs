...
use xiaomimimo_config::{CliRuntimeOverrides, ConfigStore};
use xiaomimimo_core::Runtime;
use xiaomimimo_execpolicy::ExecPolicyEngine;
use xiaomimimo_hooks::{HookDispatcher, JsonlHookSink, StdoutHookSink};
use xiaomimimo_mcp::McpManager;
use xiaomimimo_protocol::{
    AppRequest, AppResponse, PromptRequest, PromptResponse, ThreadRequest, ThreadResponse,
};
use xiaomimimo_state::StateStore;
...
struct AppState {
    ...
    config: Arc<RwLock<xiaomimimo_config::ConfigToml>>,
...
async fn tool_handler(
    ...
    match runtime
        .invoke_tool(
            req.call,
            xiaomimimo_execpolicy::AskForApproval::OnRequest,
            &cwd,
        )
        .await
    {
...
fn build_state(config_path: Option<PathBuf>) -> Result<AppState> {
    ...
    let hook_log_path = config_path
        ...
        .unwrap_or_else(|| PathBuf::from(".xiaomimimo/events.jsonl"));
    ...
    match req {
        ...
        AppRequest::ThreadLoadedList => {
            ...
            let response = runtime
                .handle_thread(xiaomimimo_protocol::ThreadRequest::List(
                    xiaomimimo_protocol::ThreadListParams {
...
async fn persist_config(state: &AppState, config: xiaomimimo_config::ConfigToml) -> Result<()> {
