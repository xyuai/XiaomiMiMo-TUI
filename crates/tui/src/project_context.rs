//! Project context loading for XiaomiMiMo TUI.
//!
...
//! - `.xiaomimimo/instructions.md` - Hidden instructions file (legacy)
//!
...
This file provides guidance to AI agents (XiaomiMiMo TUI, Claude Code, etc.) when working with code in this repository.
...
mod tests {
    ...
    fn test_load_project_context_hidden_dir() {
        let tmp = tempdir().expect("tempdir");
        let hidden_dir = tmp.path().join(".xiaomimimo");
    ...
    fn test_check_trust_status() {
        ...
        // Create trust marker
        let xiaomimimo_dir = tmp.path().join(".xiaomimimo");
        fs::create_dir(&xiaomimimo_dir).expect("mkdir");
        fs::write(xiaomimimo_dir.join("trusted"), "").expect("write");
