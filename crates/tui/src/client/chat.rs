//! Chat Completions API helpers for XiaomiMiMo's OpenAI-compatible endpoint.
//!
...
fn stream_idle_timeout() -> Duration {
    let secs = std::env::var("XIAOMIMIMO_STREAM_IDLE_TIMEOUT_SECS")
        .ok()
...
use super::{
    XiaomiMiMoClient, ERROR_BODY_MAX_BYTES, SSE_BACKPRESSURE_HIGH_WATERMARK,
...
impl XiaomiMiMoClient {
    pub(super) async fn create_message_chat(
        ...
        if !status.is_success() {
            ...
            anyhow::bail!("Failed to call XiaomiMiMo Chat API: HTTP {status}: {error_text}");
        }
...
impl XiaomiMiMoClient {
    pub(super) async fn handle_chat_completion_stream(
        ...
        // but no reasoning_content. XiaomiMiMo's thinking-mode API rejects
        ...
        if !status.is_success() {
            let error_text = bounded_error_text(response, ERROR_BODY_MAX_BYTES).await;
            // If XiaomiMiMo rejected for missing reasoning_content despite the
...
fn build_chat_messages_with_reasoning(
    ...
    for message in messages.iter() {
        ...
        if role == "assistant" {
            ...
            // XiaomiMiMo thinking-mode rule: every assistant message in the
            ...
            if include_reasoning_for_turn && !has_reasoning {
                logging::warn(
                    "Substituting placeholder reasoning_content for XiaomiMiMo tool-call assistant message",
                );
                reasoning_content = String::from("(reasoning omitted)");
                has_reasoning = true;
            }

            // XiaomiMiMo rejects assistant messages where both `content` and
    ...
    while i < out.len() {
        ...
        if is_assistant_with_tools {
            ...
            if !expected_ids.is_subset(&found_ids) {
                ...
                // assistant message entirely (XiaomiMiMo requires content or tool_calls).
...
/// XiaomiMiMo's thinking-mode API rejects such messages with a 400 error;
...
/// users on `RUST_LOG=xiaomimimo_tui=debug` can see how much of their input
...
pub(super) fn sanitize_thinking_mode_messages(
    ...
    // ~4 chars/token is the standard rough estimate; XiaomiMiMo tokens skew
...
/// Diagnostic logger fired when XiaomiMiMo rejects the request despite the
...
fn log_thinking_mode_violations(body: &Value) {
    ...
    if violations.is_empty() {
        logging::warn(
            "400-after-sanitizer: all assistant messages have reasoning_content — XiaomiMiMo rejected for a different reason",
        );
    } else {
        logging::warn(format!(
...
fn requires_reasoning_content(model: &str) -> bool {
    ...
    lower.contains("mimo-v2.5-pro")
        ...
        || has_xiaomimimo_r_series_marker(&lower)
}
...
mod stream_diagnostics_tests {
    ...
    fn format_stream_headers_marks_missing_fields_as_absent() {
        // XiaomiMiMo frequently omits content-encoding when not compressing.
...
mod stream_decoder_tests {
    ...
    fn decoder_yields_no_events_for_keepalive_chunk() {
        // XiaomiMiMo often sends `{"choices":[]}` keepalive chunks before
    ...
    fn decoder_emits_tool_use_block_for_tool_call_delta() {
        ...
        // path that XiaomiMiMo has billed for).
