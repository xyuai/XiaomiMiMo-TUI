...
//!
//! Storage: `~/.xiaomimimo/workspace-trust.json`. The file is a JSON object
...
//! through XiaomiMiMo-TUI's own file tools (`read_file`, `write_file`, etc.) —
...
fn trust_file_path() -> Option<PathBuf> {
    dirs::home_dir().map(|home| home.join(".xiaomimimo").join(TRUST_FILE_NAME))
}
...
mod tests {
    ...
    /// Set up an isolated fake `~/.xiaomimimo/workspace-trust.json` location.
    ...
    fn isolated_trust_path() -> (TempDir, PathBuf) {
        ...
        let trust_path = tmp.path().join(".xiaomimimo").join("workspace-trust.json");
        (tmp, trust_path)
    }
