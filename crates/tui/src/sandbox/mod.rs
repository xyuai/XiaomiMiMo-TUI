...
//! XiaomiMiMo TUI. Sandboxing restricts what system resources a command can access,
...
impl SandboxManager {
    ...
    fn prepare_seatbelt(spec: &CommandSpec) -> ExecEnv {
        ...
        env.insert("XIAOMIMIMO_SANDBOX".to_string(), "seatbelt".to_string());
    ...
    fn prepare_landlock(spec: &CommandSpec) -> ExecEnv {
        ...
        // Add sandbox indicator to environment
        let mut env = spec.env.clone();
        env.insert("XIAOMIMIMO_SANDBOX".to_string(), "landlock".to_string());
    ...
    fn prepare_windows(spec: &CommandSpec) -> ExecEnv {
        ...
        env.insert("XIAOMIMIMO_SANDBOX".to_string(), format!("windows:{kind}"));
        if !spec.sandbox_policy.has_network_access() {
            env.insert(
                "XIAOMIMIMO_SANDBOX_BLOCK_NETWORK".to_string(),
                "1".to_string(),
            );
        }
