...
pub enum SandboxPolicy {
    ...
    ///
    /// Use this when XiaomiMiMo TUI is itself running inside a container,
            ...
            SandboxPolicy::WorkspaceWrite {
                ...
                roots
                    ...
                    .map(|root| {
                        ...
                        // Protect .xiaomimimo directories from modification
                        let xiaomimimo_dir = root.join(".xiaomimimo");
                        if xiaomimimo_dir.is_dir() {
                            read_only_subpaths.push(xiaomimimo_dir);
                        }
...
///
/// This allows fine-grained control like "allow writes to /project but not /project/.xiaomimimo".
...
mod tests {
    ...
    fn test_writable_root_with_exceptions() {
        let root = WritableRoot::with_exceptions(
            ...
            vec![PathBuf::from("/project/.xiaomimimo")],
        );
        assert!(root.is_path_writable(Path::new("/project/src/main.rs")));
        assert!(!root.is_path_writable(Path::new("/project/.xiaomimimo/config")));
    }
