...
fn render_modal_chrome(area: Rect, popup_area: Rect, buf: &mut Buffer) {
    ...
    if shadow_width > 0 && shadow_height > 0 {
        Block::default()
            .style(Style::default().bg(palette::XIAOMIMIMO_NAVY))
            .render(
                Rect {
                    x: shadow_x,
                    y: shadow_y,
...
impl ModalView for UserInputView {
    ...
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        if self.mode == InputMode::OtherInput {
            lines.push(Line::from(vec![
                Span::styled("Enter", Style::default().fg(palette::XIAOMIMIMO_SKY).bold()),
                Span::styled(" submit", Style::default().fg(palette::TEXT_MUTED)),
                Span::raw("  "),
                Span::styled("Esc", Style::default().fg(palette::XIAOMIMIMO_SKY).bold()),
                Span::styled(" back", Style::default().fg(palette::TEXT_MUTED)),
            ]));
        } else {
            lines.push(Line::from(vec![
                Span::styled("1-4", Style::default().fg(palette::XIAOMIMIMO_SKY).bold()),
                Span::styled(" quick pick", Style::default().fg(palette::TEXT_MUTED)),
                Span::raw("  "),
                Span::styled("Up/Down", Style::default().fg(palette::XIAOMIMIMO_SKY).bold()),
