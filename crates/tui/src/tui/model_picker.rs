//! `/model` picker modal: pick a XiaomiMiMo model and a thinking-effort tier
...
//! Xiaomi MiMo docs, thinking is controlled by `thinking.type`; low/medium/high/max
//! all enable thinking in this TUI while Off disables it.
//! The legacy variants remain valid in `~/.xiaomimimo/settings.toml` for
...
const PICKER_MODELS: &[(&str, &str)] = &[
    ("mimo-v2.5-pro", "flagship / coding"),
    ("mimo-v2.5", "general 1M"),
...
mod tests {
    ...
    fn create_test_app() -> App {
        ...
        // App::new merges in `~/.config/xiaomimimo/settings.toml` /
        // `Application Support/xiaomimimo/settings.toml`, which can override
    ...
    fn picker_preserves_unknown_model_via_custom_row() {
        let mut app = create_test_app();
        app.model = "mimo-v2.5-pro-2026-04-XX".to_string();
        let view = ModelPickerView::new(&app);
        assert!(view.show_custom_model_row);
        assert_eq!(view.resolved_model(), "mimo-v2.5-pro-2026-04-XX");
    }
