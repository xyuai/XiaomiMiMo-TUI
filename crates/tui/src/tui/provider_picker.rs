//! `/provider` picker modal — pick a provider (XiaomiMiMo / NVIDIA NIM /
...
mod tests {
    ...
    fn enter_with_existing_key_emits_apply_and_closes() {
        let config = Config {
            api_key: Some("existing-xiaomimimo-key".to_string()),
            ..Config::default()
        };
        let mut picker = ProviderPickerView::new(ApiProvider::NvidiaNim, &config);
        // Move up to XiaomiMiMo (index 0), which has a key from the config.
