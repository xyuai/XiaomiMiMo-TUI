    ...
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        if self.agents.is_empty() {
            ...
            let status_summary = [
                ...
                ("Failed", failed.len(), palette::XIAOMIMIMO_RED),
    ...
    for agent in agents {
        ...
        if let Some(detail) = status_detail {
            ...
            lines.push(Line::from(vec![
                ...
                Span::styled(detail, Style::default().fg(palette::XIAOMIMIMO_RED)),
            ]));
        }
        ...
        if let Some(role) = agent.assignment.role.as_deref() {
            ...
            lines.push(Line::from(vec![
                ...
                Span::styled(role, Style::default().fg(palette::XIAOMIMIMO_SKY)),
            ]));
        }
...
fn format_agent_status(
    ...
    match status {
        ...
        SubAgentStatus::Failed(reason) => (
            "failed",
            Style::default().fg(palette::XIAOMIMIMO_RED),
            Some(reason.as_str()),
        ),
    }
}
