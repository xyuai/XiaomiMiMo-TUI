...
pub struct HeaderData<'a> {
    ...
    /// Legacy accumulated session cost kept for hooks/compat; header does not render it.
    ...
    /// default-XiaomiMiMo case), no provider chip is rendered. Surfaces the
    /// fact that requests are going somewhere other than XiaomiMiMo's API so
...
impl<'a> HeaderData<'a> {
    ...
    /// the default XiaomiMiMo provider so the chip is hidden.
    #[must_use]
...
mod tests {
    ...
    fn wide_header_shows_plain_mode_and_single_metadata_cluster() {
        let rendered = render_header(
            HeaderData::new(
                ...
                "xiaomimimo-tui",
                false,
                palette::XIAOMIMIMO_INK,
            ),
            72,
        );

        assert!(rendered.contains("Agent"));
        assert!(rendered.contains("xiaomimimo-tui"));
        assert!(rendered.contains("mimo-v2.5-pro"));
    ...
    fn narrow_header_keeps_context_percent_visible() {
        let rendered = render_header(
            HeaderData::new(AppMode::Agent, "", "", true, palette::XIAOMIMIMO_INK).with_usage(
                0,
                Some(128_000),
                0.0,
                Some(48_000),
    ...
    #[test]
    fn header_hides_provider_chip_when_default_xiaomimimo() {
        let rendered = render_header(
            HeaderData::new(
                AppMode::Agent,
                "mimo-v2.5-pro",
