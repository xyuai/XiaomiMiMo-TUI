            ...
            while let Ok(event) = rx.try_recv() {
                ...
                match event {
                    ...
                    EngineEvent::MessageComplete { .. } => {
                        ...
                        // XiaomiMiMo rejects assistant messages that contain only reasoning blocks.
...
/// `~/.xiaomimimo/settings.toml` so it survives a restart, push the change to
...
/// Persist the typed API key to `~/.xiaomimimo/config.toml`, refresh the
...
fn status_color(level: StatusToastLevel) -> ratatui::style::Color {
    match level {
        StatusToastLevel::Info => palette::XIAOMIMIMO_SKY,
...
fn render_footer(f: &mut Frame, area: Rect, app: &mut App) {
    ...
    if footer_working_strip_active(app) {
        ...
        props.state_color = palette::XIAOMIMIMO_SKY;
...
fn footer_coherence_spans(app: &App) -> Vec<Span<'static>> {
    ...
    let (label, color) = match app.coherence_state {
        ...
        CoherenceState::VerifyingRecentWork => ("verifying", palette::XIAOMIMIMO_SKY),
...
fn footer_state_label(app: &App) -> (&'static str, ratatui::style::Color) {
    ...
    if running_agent_count(app) > 0 {
        return ("working", palette::XIAOMIMIMO_SKY);
    }
