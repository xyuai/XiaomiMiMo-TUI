...
//! encoded as PNG and persisted under `~/.xiaomimimo/clipboard-images/` so the
//! model can reach them via the existing `@`-mention / file tools (XiaomiMiMo
...
impl ClipboardHandler {
    ...
    ///
    /// `workspace` is used as a fallback location when `~/.xiaomimimo/` cannot
...
/// `~/.xiaomimimo/clipboard-images/` so the path is stable across worktrees and
...
fn clipboard_images_dir(workspace: &Path) -> PathBuf {
    if let Some(home) = dirs::home_dir() {
        return home.join(".xiaomimimo").join("clipboard-images");
    }
...
mod tests {
    ...
    fn osc52_sequence_encodes_text() {
        let sequence = osc52_sequence("hello", false).expect("osc52");
        assert_eq!(sequence, "\x1b]52;c;aGVsbG8=\x07");
    }

    #[test]
    fn osc52_sequence_wraps_for_tmux() {
        let sequence = osc52_sequence("hello", true).expect("osc52");
