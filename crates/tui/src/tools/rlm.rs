...
const DEFAULT_CHILD_MODEL: &str = "mimo-v2-flash";
...
impl RlmTool {
    #[must_use]
    pub fn new(client: Option<XiaomiMiMoClient>, root_model: String) -> Self {
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "child_model": {
                    "type": "string",
                    "description": "Model for sub-LLM (`llm_query`) calls inside the REPL. Default: mimo-v2-flash."
                },
                "max_depth": {
                    "type": "integer",
    ...
    async fn execute(&self, input: Value, context: &ToolContext) -> Result<ToolResult, ToolError> {
        let Some(client) = self.client.clone() else {
            return Err(ToolError::not_available(
                "rlm_process requires an active XiaomiMiMo client".to_string(),
            ));
        };
...
mod tests {
    ...
    fn tool() -> RlmTool {
        RlmTool::new(None, "mimo-v2.5-pro".to_string())
    }
