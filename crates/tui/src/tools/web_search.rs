...
mod tests {
    ...
    fn extract_search_query_accepts_legacy_query() {
        let query =
            extract_search_query(&json!({"query": " xiaomimimo v4 "})).expect("query should parse");
        assert_eq!(query, "xiaomimimo v4");
    }

    #[test]
    fn extract_search_query_accepts_q_alias() {
        let query =
            extract_search_query(&json!({"q": "xiaomimimo v4 pro"})).expect("q alias should parse");
        assert_eq!(query, "xiaomimimo v4 pro");
    }

    #[test]
    fn extract_search_query_accepts_array_form() {
        let input = json!({"search_query": [{"q": "xiaomimimo api", "max_results": 3}]});
        let query = extract_search_query(&input).expect("array form should parse");
        assert_eq!(query, "xiaomimimo api");
