...
impl ReviewTool {
    #[must_use]
    pub fn new(client: Option<XiaomiMiMoClient>, model: String) -> Self {
        Self { client, model }
    }
}
    ...
    async fn execute(&self, input: Value, context: &ToolContext) -> Result<ToolResult, ToolError> {
        let Some(client) = self.client.clone() else {
            return Err(ToolError::not_available(
                "Review tool requires an active XiaomiMiMo client".to_string(),
            ));
        };
...
mod tests {
    ...
    fn parses_pr_url() {
        let pr =
            parse_pr_url("https://github.com/xiaomimimo-ai/xiaomimimo-cli/pull/123").expect("parse pr");
        assert_eq!(pr.owner, "xiaomimimo-ai");
        assert_eq!(pr.repo, "xiaomimimo-cli");
        assert_eq!(pr.number, "123");
    }

    #[test]
    fn ignores_non_pr_url() {
        assert!(parse_pr_url("https://github.com/xiaomimimo-ai/xiaomimimo-cli").is_none());
