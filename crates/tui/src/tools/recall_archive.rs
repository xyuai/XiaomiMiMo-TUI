...
//! on-disk JSONL archives at `~/.xiaomimimo/sessions/<id>/cycles/*.jsonl` and
...
//! `~/.xiaomimimo/embeddings/` cache built on archive write.
...
mod tests {
    ...
    fn list_archives_sorts_by_cycle_number() {
        ...
        let sid = fresh_session_id();
        let now = Utc::now();
        archive_cycle(&sid, 3, &[user_msg("c3")], "mimo-v2.5-pro", now).unwrap();
        archive_cycle(&sid, 1, &[user_msg("c1")], "mimo-v2.5-pro", now).unwrap();
        archive_cycle(&sid, 2, &[user_msg("c2")], "mimo-v2.5-pro", now).unwrap();
    ...
    async fn execute_finds_matching_messages() {
        ...
        archive_cycle(&sid, 1, &messages, "mimo-v2.5-pro", now).unwrap();
