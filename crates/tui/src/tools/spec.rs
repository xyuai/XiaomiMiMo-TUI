//! Tool specification traits for the XiaomiMiMo TUI agent system.
//!
...
pub struct ToolContext {
    ...
    /// fall outside `workspace`. Loaded from `~/.xiaomimimo/workspace-trust.json`
...
impl ToolContext {
    ...
    pub fn new(workspace: impl Into<PathBuf>) -> Self {
        ...
        let notes_path = workspace.join(".xiaomimimo").join("notes.md");
        let mcp_config_path = workspace.join(".xiaomimimo").join("mcp.json");
        Self {
            workspace,
            shell_manager,
            trust_mode: false,
    ...
    /// `~/.xiaomimimo/workspace-trust.json`). See [`Self::resolve_path`] for
    ...
    pub fn resolve_path(&self, raw: &str) -> Result<PathBuf, ToolError> {
        ...
        // `~/.xiaomimimo/workspace-trust.json`).
        if !canonical.starts_with(&workspace_canonical)
