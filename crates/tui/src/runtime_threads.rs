...
//!
//! This module keeps XiaomiMiMo-only execution while exposing Codex-like lifecycle
...
/// files at `~/.xiaomimimo/sessions/<id>/cycles/<n>.jsonl`. A v1 reader on a
...
impl RuntimeThreadManagerConfig {
    ...
    pub fn from_task_data_dir(task_data_dir: PathBuf) -> Self {
        let data_dir = if let Ok(override_dir) = std::env::var("XIAOMIMIMO_RUNTIME_DIR") {
...
mod tests {
    ...
    fn test_runtime_dir() -> PathBuf {
        std::env::temp_dir().join(format!("xiaomimimo-runtime-threads-{}", Uuid::new_v4()))
    }
