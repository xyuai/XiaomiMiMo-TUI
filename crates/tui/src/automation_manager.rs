...
//! `~/.xiaomimimo/automations` (or `XIAOMIMIMO_AUTOMATIONS_DIR` override).
...
pub fn default_automations_dir() -> PathBuf {
    if let Ok(path) = std::env::var("XIAOMIMIMO_AUTOMATIONS_DIR") {
    ...
    dirs::home_dir()
        .map(|home| home.join(".xiaomimimo").join("automations"))
        .unwrap_or_else(|| PathBuf::from(".xiaomimimo").join("automations"))
}
