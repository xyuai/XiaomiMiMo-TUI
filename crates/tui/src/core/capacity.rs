...
impl Default for CapacityControllerConfig {
    fn default() -> Self {
        ...
        model_priors.insert("xiaomimimo_v3_2_chat".to_string(), 3.9);
        model_priors.insert("xiaomimimo_v3_2_reasoner".to_string(), 4.1);
        model_priors.insert("xiaomimimo_v4_pro".to_string(), 3.5);
        model_priors.insert("xiaomimimo_v4_flash".to_string(), 4.2);
        ...
        Self {
            ...
            // via `capacity.enabled = true` in `~/.xiaomimimo/config.toml`.
            enabled: false,
...
impl CapacityControllerConfig {
    ...
    pub fn from_app_config(config: &crate::config::Config) -> Self {
        ...
        if let Some(v) = capacity.xiaomimimo_v3_2_reasoner_prior {
            out.model_priors
                .insert("xiaomimimo_v3_2_reasoner".to_string(), v);
        }
        if let Some(v) = capacity.xiaomimimo_v4_pro_prior {
...
mod tests {
    ...
    fn app_config_without_capacity_keeps_controller_disabled() {
        ...
        assert_eq!(cfg.model_priors.get("xiaomimimo_v4_pro"), Some(&3.5));
        assert_eq!(cfg.model_priors.get("xiaomimimo_v4_flash"), Some(&4.2));
    }

    #[test]
    fn normalize_v4_pro_variants() {
        assert_eq!(
            normalize_model_prior_key("mimo-v2.5-pro"),
            "xiaomimimo_v4_pro"
        );
        assert_eq!(
            normalize_model_prior_key("mimo-v2.5-pro_pro"),
            "xiaomimimo_v4_pro"
        );
        assert_eq!(
            normalize_model_prior_key("mimo-v2.5-pro"),
        ...
        assert_eq!(
            normalize_model_prior_key("xiaomimimo-ai/mimo-v2.5-pro_pro"),
            "xiaomimimo_v4_pro"
        );
    }
    ...
    fn normalize_v4_flash_variants() {
        assert_eq!(
            normalize_model_prior_key("mimo-v2-flash"),
            "xiaomimimo_v4_flash"
        );
        assert_eq!(
            normalize_model_prior_key("mimo-v2.5-pro_flash"),
            "xiaomimimo_v4_flash"
        );
        assert_eq!(
            normalize_model_prior_key("mimo-v2-flash"),
        ...
        assert_eq!(
            normalize_model_prior_key("xiaomimimo-ai/mimo-v2.5-pro_flash"),
            "xiaomimimo_v4_flash"
        );
    }
    ...
    fn normalize_v3_and_reasoner_unchanged() {
        assert_eq!(
            normalize_model_prior_key("xiaomimimo-chat"),
            "xiaomimimo_v3_2_chat"
        );
        assert_eq!(
            normalize_model_prior_key("xiaomimimo-v3-chat"),
            "xiaomimimo_v3_2_chat"
        );
        assert_eq!(
            normalize_model_prior_key("xiaomimimo-reasoner"),
            "xiaomimimo_v3_2_reasoner"
        );
        assert_eq!(
            normalize_model_prior_key("xiaomimimo-r1"),
            "xiaomimimo_v3_2_reasoner"
        );
        assert_eq!(
    ...
    ///
    /// ```text
    /// cargo test -p xiaomimimo-tui --release capacity::tests::bench_compute_profile -- --ignored --nocapture
    /// ```
    ///
