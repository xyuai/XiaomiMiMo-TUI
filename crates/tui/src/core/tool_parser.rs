//! Legacy parser for text-based tool calls from XiaomiMiMo models.
//!
//! Structured tool-call items are preferred, so the engine no longer invokes
//! this parser. It is kept for reference/debugging.
//!
//! Some XiaomiMiMo outputs tool calls as text in various formats:
//! ```text
//! [TOOL_CALL]
//! {tool => "tool_name", args => {...}}
//! [/TOOL_CALL]
...
fn get_xml_tool_call_regex() -> &'static Regex {
    XML_TOOL_CALL_REGEX.get_or_init(|| {
        // Match <xiaomimimo:tool_call>...</xiaomimimo:tool_call> or similar XML patterns
        Regex::new(r"(?s)<(?:xiaomimimo:)?tool_call[^>]*>\s*(.*?)\s*</(?:xiaomimimo:)?tool_call>")
...
pub fn parse_tool_calls(text: &str) -> ParseResult {
    ...
    // Parse XML-style <xiaomimimo:tool_call> or <tool_call> format
...
pub fn has_tool_call_markers(text: &str) -> bool {
    text.contains("[TOOL_CALL]")
        || text.contains("<xiaomimimo:tool_call")
        || text.contains("<tool_call")
