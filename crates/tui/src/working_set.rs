...
impl Workspace {
    ...
    fn build_file_index(&self) -> HashMap<String, Vec<PathBuf>> {
        ...
        // Honor `.xiaomimimoignore` in addition to the defaults the `ignore` crate
    ...
    /// hits sort first so `docs/de` lands `docs/xiaomimimo_v4.pdf` ahead of any
    ...
    /// `.xiaomimimoignore`. Capped at `limit` results.
    #[must_use]
