...
//!    `~/.xiaomimimo/audit.log` in the format described below.
//!
...
//! # Host-matching rules
//!
//! * **Exact match** — an entry like `token-plan-cn.xiaomimimo.com` matches only the host
//!   `token-plan-cn.xiaomimimo.com` (case-insensitive).
...
//! `~/.xiaomimimo/audit.log`). Best-effort: write failures are logged but do
//! not block the call.
...
impl NetworkAuditor {
    ...
    /// Auditor pointing at `~/.xiaomimimo/audit.log`. Returns `None` if the
...
impl NetworkPolicyDecider {
    ...
    /// `~/.xiaomimimo/audit.log`, if `policy.audit` is true.
    #[must_use]
...
mod tests {
    ...
    fn exact_match_in_allow_returns_allow() {
        let p = mk(Decision::Deny, &["token-plan-cn.xiaomimimo.com"], &[]);
        assert_eq!(p.decide("token-plan-cn.xiaomimimo.com"), Decision::Allow);
    }
    ...
    fn host_from_url_extracts_host() {
        assert_eq!(
            host_from_url("https://token-plan-cn.xiaomimimo.com/v1/health"),
            Some("token-plan-cn.xiaomimimo.com".to_string())
        );
        assert_eq!(
    ...
    fn audit_records_terminal_decisions_through_decider() {
        ...
        let policy = mk(Decision::Deny, &["token-plan-cn.xiaomimimo.com"], &[]);
        let decider = NetworkPolicyDecider::new(policy, Some(auditor));

        let allow = decider.evaluate("token-plan-cn.xiaomimimo.com", "fetch_url");
