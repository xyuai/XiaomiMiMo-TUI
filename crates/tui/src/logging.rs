...
pub fn env_requests_verbose_logging() -> bool {
    std::env::var("XIAOMIMIMO_LOG_LEVEL")
        .ok()
...
mod tests {
    ...
    fn log_value_parser_accepts_common_rust_log_directives() {
        ...
        assert!(log_value_enables_verbose("xiaomimimo_cli=debug"));
        assert!(log_value_enables_verbose("warn,xiaomimimo_tui::client=trace"));
        assert!(!log_value_enables_verbose("warn"));
        assert!(!log_value_enables_verbose("xiaomimimo_tui=off"));
    }
}
