//! Text chat workflows for `XiaomiMiMo` and XiaomiMiMo-compatible APIs.
...
pub async fn run_xiaomimimo_chat(client: &XiaomiMiMoClient, options: TextChatOptions) -> Result<()> {
    let mut messages: Vec<Message> = Vec::new();
    let mut stats = SessionStats::new();

    print_banner("XiaomiMiMo Compatible API");
    print_session_info(
        &options,
        messages.len(),
    ...
    if let Some(prompt) = options.prompt.as_deref() {
        process_xiaomimimo_turn(client, &options, &mut messages, prompt, &mut stats).await?;
    } else {
        let mut rl = create_editor()?;
        while let Some(line) = read_prompt(&mut rl)? {
            if handle_line_xiaomimimo(line, client, &options, &mut messages, &mut stats).await? {
                break;
            }
        }
    }
...
pub async fn run_official_chat(client: &XiaomiMiMoClient, options: TextChatOptions) -> Result<()> {
...
async fn process_xiaomimimo_turn(
...
fn print_banner(mode: &str) {
    println!("{}", ds_blue("XiaomiMiMo TUI").bold());
    println!("Mode: {mode}");
...
fn ds_blue(text: &str) -> ColoredString {
    let (r, g, b) = palette::XIAOMIMIMO_BLUE_RGB;
    text.truecolor(r, g, b)
}

fn ds_sky(text: &str) -> ColoredString {
    let (r, g, b) = palette::XIAOMIMIMO_SKY_RGB;
    text.truecolor(r, g, b)
}

fn ds_red(text: &str) -> ColoredString {
    let (r, g, b) = palette::XIAOMIMIMO_RED_RGB;
    text.truecolor(r, g, b)
}
...
fn history_path() -> Option<std::path::PathBuf> {
    dirs::home_dir().map(|home| {
        let dir = home.join(".xiaomimimo");
        let _ = std::fs::create_dir_all(&dir);
        dir.join("history")
    })
}

async fn handle_line_xiaomimimo(
    line: String,
    client: &XiaomiMiMoClient,
    options: &TextChatOptions,
    messages: &mut Vec<Message>,
    ...
    if handle_command_xiaomimimo(input, messages, Some(options), stats) {
        return Ok(false);
    }
    if let Err(error) = process_xiaomimimo_turn(client, options, messages, input, stats).await {
