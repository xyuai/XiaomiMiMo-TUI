//! Persistent background task manager for XiaomiMiMo agent work.
//!
//! Tasks are durable across restarts and execute with a bounded worker pool.
//! Execution stays XiaomiMiMo-only and now links every task to runtime
...
/// Engine-backed executor (XiaomiMiMo-only).
...
impl TaskManager {
    /// Start the manager with the default XiaomiMiMo executor.
...
/// Default task persistence location (`~/.xiaomimimo/tasks`).
#[must_use]
pub fn default_tasks_dir() -> PathBuf {
    if let Ok(path) = std::env::var("XIAOMIMIMO_TASKS_DIR")
        && !path.trim().is_empty()
    {
        return PathBuf::from(path);
    }
    if let Some(home) = dirs::home_dir() {
        return home.join(".xiaomimimo").join("tasks");
    }
    PathBuf::from(".xiaomimimo").join("tasks")
}
                ...
                &serde_json::json!({
                    "task_updates": {
                        "gate": {
                            "id": "gate_test",
                            "gate": "test",
                            "command": "cargo test -p xiaomimimo-tui --lib",
                            "cwd": ".",
                            "exit_code": 0,
                            "status": "passed",
