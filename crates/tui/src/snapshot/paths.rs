...
//!
//! Snapshots live in `~/.xiaomimimo/snapshots/<project_hash>/<worktree_hash>/`.
...
///
/// Returns `~/.xiaomimimo/snapshots/<project_hash>/<worktree_hash>/`. The
...
/// Used by tests so we never touch the user's real `~/.xiaomimimo/`.
...
mod tests {
    ...
    #[test]
    fn snapshot_dir_layout_two_levels_under_xiaomimimo() {
        ...
        assert_eq!(iter.next().unwrap().as_os_str(), ".xiaomimimo");
