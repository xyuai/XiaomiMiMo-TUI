...
//! replace or rewrite messages, which breaks XiaomiMiMo V4's prefix cache
...
pub const DEFAULT_SEAM_MODEL: &str = "mimo-v2-flash";
...
impl SeamManager {
    ...
    pub fn new(flash_client: XiaomiMiMoClient, config: SeamConfig) -> Self {
        Self {
            flash_client,
            config,
...
mod tests {
    ...
    fn seam_levels_fire_in_order() {
        // Cannot create XiaomiMiMoClient without API key in test env.
