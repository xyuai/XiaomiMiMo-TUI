...
//! ## Why
//!
//! XiaomiMiMo V4's empirical retrieval degradation begins around the 256K band
...
//!   tune the threshold separately for `mimo-v2.5-pro` vs. `-flash`.
...
pub struct CycleConfig {
    ...
    pub briefing_max_tokens: usize,
    /// Per-model overrides keyed by model identifier (e.g. `mimo-v2.5-pro`).
...
impl Default for CycleConfig {
    fn default() -> Self {
        ...
        per_model.insert("mimo-v2.5-pro".to_string(), ModelCycleConfig::default());
        per_model.insert("mimo-v2-flash".to_string(), ModelCycleConfig::default());
        Self {
            enabled: true,
            threshold_tokens: DEFAULT_CYCLE_THRESHOLD_TOKENS,
            briefing_max_tokens: DEFAULT_BRIEFING_MAX_TOKENS,
...
/// Resolve the on-disk archive directory: `~/.xiaomimimo/sessions/<id>/cycles`.
...
mod tests {
    ...
    fn cycle_config_default_includes_v4_overrides() {
        ...
        assert!(cfg.enabled);
        assert!(cfg.per_model.contains_key("mimo-v2.5-pro"));
        assert!(cfg.per_model.contains_key("mimo-v2-flash"));
    ...
    fn threshold_for_falls_back_to_default() {
        let cfg = CycleConfig::default();
        assert_eq!(
            cfg.threshold_for("mimo-v2.5-pro"),
    ...
    fn threshold_for_uses_per_model_override() {
        ...
        assert_eq!(cfg.threshold_for("mimo-v2.5-pro"), 80_000);
        assert_eq!(cfg.briefing_max_for("mimo-v2.5-pro"), 2_000);
    }
    ...
    fn archive_cycle_writes_jsonl_with_header_and_messages() {
        ...
        let started = Utc::now();
        let path = archive_cycle(&session_id, 1, &messages, "mimo-v2.5-pro", started)
        ...
        assert_eq!(header.model, "mimo-v2.5-pro");
