...
use xiaomimimo_config::{CliRuntimeOverrides, ConfigToml, ProviderKind};
use xiaomimimo_execpolicy::{
    AskForApproval, ExecApprovalRequirement, ExecPolicyContext, ExecPolicyDecision,
    ExecPolicyEngine,
};
use xiaomimimo_hooks::{HookDispatcher, HookEvent};
use xiaomimimo_mcp::{
    McpManager, McpStartupCompleteEvent, McpStartupStatus as McpManagerStartupStatus,
};
use xiaomimimo_protocol::{
...
use xiaomimimo_tools::{ToolCall, ToolRegistry};
...
impl ThreadManager {
    ...
    pub fn spawn_thread_with_history(
        ...
        let thread = Thread {
            ...
            source: match source {
                SessionSource::Interactive => xiaomimimo_protocol::SessionSource::Interactive,
                SessionSource::Resume => xiaomimimo_protocol::SessionSource::Resume,
                SessionSource::Fork => xiaomimimo_protocol::SessionSource::Fork,
                SessionSource::Api => xiaomimimo_protocol::SessionSource::Api,
                SessionSource::Unknown => xiaomimimo_protocol::SessionSource::Unknown,
            },
            name: None,
        };
    ...
    pub async fn mcp_startup(&self) -> McpStartupCompleteEvent {
        ...
        for update in updates {
            let status = match update.status {
                McpManagerStartupStatus::Starting => xiaomimimo_protocol::McpStartupStatus::Starting,
                McpManagerStartupStatus::Ready => xiaomimimo_protocol::McpStartupStatus::Ready,
                McpManagerStartupStatus::Failed { error } => {
                    xiaomimimo_protocol::McpStartupStatus::Failed { error }
                }
                McpManagerStartupStatus::Cancelled => {
                    xiaomimimo_protocol::McpStartupStatus::Cancelled
                }
            };
            self.hooks
                .emit(HookEvent::GenericEventFrame {
                    frame: EventFrame::McpStartupUpdate {
                        update: xiaomimimo_protocol::McpStartupUpdateEvent {
        ...
        self.hooks
            .emit(HookEvent::GenericEventFrame {
                frame: EventFrame::McpStartupComplete {
                    summary: xiaomimimo_protocol::McpStartupCompleteEvent {
                        ready: summary.ready.clone(),
                        failed: summary
                            .failed
                            .iter()
                            .map(|f| xiaomimimo_protocol::McpStartupFailure {
...
fn to_persisted_source(source: &xiaomimimo_protocol::SessionSource) -> SessionSource {
    match source {
        xiaomimimo_protocol::SessionSource::Interactive => SessionSource::Interactive,
        xiaomimimo_protocol::SessionSource::Resume => SessionSource::Resume,
        xiaomimimo_protocol::SessionSource::Fork => SessionSource::Fork,
        xiaomimimo_protocol::SessionSource::Api => SessionSource::Api,
        xiaomimimo_protocol::SessionSource::Unknown => SessionSource::Unknown,
    }
}
...
fn tool_output_value(output: &xiaomimimo_protocol::ToolOutput) -> Value {
